...
SIM="iPhone 6s"
UDID="27D0349A-FB3D-4547-8DC2-0878CC8DA5B0"
...
# booted=( $(xcrun simctl list | sed -n 's/.*(\(.*\)) (Shutdown)/\1/p') )
function get_uudi()
{
    echo $1
    booted=( $(xcrun simctl list | sed -n "s/.$1 (\(.*\)) (Shutdown)/\1/p") )
    if [ ${#booted[@]} != 0 ]; then
...
# kill "Simulator" --args -CurrentDeviceUDID "$UDID"